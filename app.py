...
class Config:
    ...
    # Color de las rayas usando el mismo amarillo pero sólido (1.0)
    PATTERN_COLOR = "rgba(245, 182, 22, 1.0)" 
...
class ChartBuilder:
        ...
        for i in range(num_frames):
            ...
            frame_data = [
                go.Scatter(
                    ...
                    fillpattern=dict(shape="/", fgcolor=Config.PATTERN_COLOR, bgcolor=Config.COLOR_AREA_FILL, size=5),
                    visible=True 
                ),
    ...
    def draw_fan_animated(v_points, u0_measured, u_nominal, measured_y, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_fan=True, show_sweep=True):
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines', line=dict(color=Config.COLOR_MEASURED, width=3), 
            hoverinfo='skip', visible=show_sweep
        ))
    ...
    def draw_area_shifting_demo(v_points, u_nominal, m_y_start, u0_end, ref_delta_pivot, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_limit_line=False, show_sweep=True):
        ...
        # --- TRAZA 2: Linea Roja y Relleno Verde respetando toggle ---
        fig.add_trace(go.Scatter(
            x=v_points, y=m_y_start, mode='lines', fill='tozeroy', 
            fillcolor=Config.COLOR_AREA_FILL, 
            fillpattern=dict(shape="/", fgcolor=Config.PATTERN_COLOR, bgcolor=Config.COLOR_AREA_FILL, size=5),
        ...
        for i in range(num_frames):
            ...
            # Aseguramos que los frames también tengan la propiedad fillpattern si se quiere
            frame_data.append(go.Scatter(x=v_points, y=curr_m_y, text=curr_m_labels, visible=True))
            
            # Traza de relleno en frame
            frame_data.append(go.Scatter(
                x=v_points, y=curr_m_y, 
                fillpattern=dict(shape="/", fgcolor=Config.PATTERN_COLOR, bgcolor=Config.COLOR_AREA_FILL, size=5),
                visible=True
            )) 
            
            frames.append(go.Frame(data=frame_data, name=str(i), traces=[0, 1, 2]))
        ...
        title_text = (
            ...
            f"U0 ≤ {str(u0_start).replace('.',',')}·10<sup>-3</sup> & [Tgδ@1.5U0 – Tgδ@0.5U0] ≤ {str(ref_delta_pivot).replace('.',',')}x10<sup>-3</sup>"
        )
        ticks_v, ticks_l = get_ticks_labels(v_points)
        ChartBuilder._add_dynamic_titles_and_axis(fig, title_text, ticks_v, ticks_l, num_frames, y_max, y_dtick)
