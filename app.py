...
def main():
    st.markdown(
        ...
        /* Transformar el radio button en pestañas */
        div[role="radiogroup"] {
            flex-direction: row;
            gap: 4px; /* Espacio entre pestañas */
        }
        
        /* Ocultar definitivamente el círculo de selección */
        div[role="radiogroup"] label > div:first-child {
            display: none !important;
        }
        
        /* Estilo base de las pestañas (inactivas) */
        div[role="radiogroup"] label {
            background-color: #F0F2F6;
            padding: 10px 24px;
            border-radius: 8px 8px 0px 0px; /* Bordes redondeados arriba */
            cursor: pointer;
            margin: 0;
        ...
        div[role="radiogroup"] label[aria-checked="true"] {
            background-color: #D3D9DF !important; /* Gris institucional */
            font-weight: bold;
            border-top: 3px solid #D50000 !important; /* Acento rojo Inducor arriba */
            box-shadow: 0px 4px 0px #D3D9DF; /* Truco para fusionar con el fondo */
        }
        
        /* Ajustar el texto dentro de la pestaña para que quede centrado */
    ...
    # --- ZONA SUPERIOR PRINCIPAL: SELECCIÓN DE MODO ---
    st.markdown('<p class="main-header">Modo de Visualización</p>', unsafe_allow_html=True)
    
    # Usamos st.radio invisiblemente para controlar el backend
    ...
    # Línea divisoria debajo de las pestañas para mayor realismo
    st.markdown("<hr style='margin-top: 0; border-color: #E0E0E0;'>", unsafe_allow_html=True)

    # --- SIDEBAR: PARÁMETROS TÉCNICOS (Comunes a ambos modos) ---
    ...
    with st.sidebar.expander("Ajuste eje X en kV", expanded=False):
        kv_start = st.number_input("Tensión Inicial (0,5x10⁻³Uo)", value=v05_def, format="%.2f", min_value=0.0)
        kv_mid = st.number_input("Tensión Inicial (1,0x10⁻³Uo)", value=u0_def, format="%.2f", min_value=0.0)
        kv_end = st.number_input("Tensión Inicial (1,5x10⁻³Uo)", value=v15_def, format="%.2f", min_value=0.0)
