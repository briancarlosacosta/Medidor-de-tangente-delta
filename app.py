...
class Config:
    ...
    COLOR_BOX_NOACTION = "#F5B616" # Amarillo Sólido para la caja
    COLOR_BOX_U0 = "#D3D9DF" # Gris Claro
...
if 'm_15_noaction' not in st.session_state: st.session_state['m_15_noaction'] = 4.0 + Config.DELTA_NO_ACTION
...
def sync_mediciones_noaction():
    ...
    st.session_state['m_15_noaction'] = float(m10 + Config.DELTA_NO_ACTION)
...
def get_ticks_labels(voltage_values: list):
    ...
    labels_stacked = [
        f"<b>{str(v05).replace('.', ',')}</b><br>0,5 Uo", 
        f"<b>{str(v10).replace('.', ',')}</b><br>Uo", 
        f"<b>{str(v15).replace('.', ',')}</b><br>1,5 Uo"
    ]
...
def generate_gif_from_frames(base_fig: go.Figure, frames) -> bytes:
    ...
    if not frames:
        ...
        img_bytes = temp_fig.to_image(format="png", width=1600, height=850, scale=1)
...
class ChartBuilder:
    ...
    def _add_dynamic_titles_and_axis(fig: go.Figure, title_text: str, x_ticks: list, x_labels: list, num_steps: int = 50, y_max: float = 10.0, y_dtick: float = 1.0):
        ...
        fig.update_layout(
            title=dict(text=f"<b>{title_text}</b>", x=0.5, xanchor='center', font=dict(size=14, family="Arial Black", color="black"), y=0.88),
    ...
    def _generate_sequential_frames_tendencia(v_points, measured_y, base_y, num_frames, trace_indices, show_sweep):
        ...
        for i in range(num_frames):
            ...
            frame_data = [
                ...
                go.Scatter(x=x_line, y=y_line, mode='lines+markers', visible=True, cliponaxis=False) 
            ]
    ...
    def draw_fan_animated(v_points, u0_measured, u_nominal, measured_y, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_fan=True, show_sweep=True):
        ...
        ticks_v, ticks_stacked, ticks_flat = get_ticks_labels(v_points)
        ChartBuilder._add_dynamic_titles_and_axis(fig, title_text, ticks_v, ticks_flat, num_frames, y_max, y_dtick)
        ...
        fig.update_layout(
            ...
            margin=dict(t=120, b=100) # Modificado: Aumentado el margen inferior para el logo
        )
        # Modificado: Se movió el logo a la esquina inferior derecha
    ...
    def draw_area_shifting_demo(v_points, u_nominal, m_y_start, u0_end, ref_delta_pivot, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_limit_line=False, show_sweep=True):
        ...
        # ==========================================
        # CONSTRUCCIÓN DEL CIRCUITO ESTÁTICO NATIVO (x3, y3)
        # ==========================================
        static_shapes = []
        c_x = 0.6 
        ...
        fig.add_trace(go.Scatter(x=v_points, y=m_y_start, mode='lines', fill='tozeroy', line=dict(width=0), fillcolor=Config.COLOR_AREA_FILL, hoverinfo='skip', visible=show_sweep, xaxis='x2', yaxis='y2', cliponaxis=False), row=1, col=2) # 0
        fig.add_trace(go.Scatter(x=v_points, y=m_y_start, mode='lines+markers+text', text=y_exact_labels, textposition="top center", marker=dict(size=12, symbol='diamond', color='white', line=dict(width=2, color='black')), line=dict(color=Config.COLOR_MEASURED, width=3), hoverinfo='skip', visible=show_sweep, xaxis='x2', yaxis='y2', cliponaxis=False), row=1, col=2) # 1 
        ...
        fig.add_trace(go.Scatter(x=[i_r_init, i_r_init], y=[0, i_c_init], mode='lines', line=dict(color='gray', width=2, dash='dash'), hoverinfo='skip', visible=show_sweep, xaxis='x', yaxis='y'), row=1, col=1) # 2
        fig.add_trace(go.Scatter(x=[0, i_r_init], y=[i_c_init, i_c_init], mode='lines', line=dict(color='gray', width=2, dash='dash'), hoverinfo='skip', visible=show_sweep, xaxis='x', yaxis='y'), row=1, col=1) # 3
        
        xw_i, yw_i, xa_i, ya_i, _, _, _, _ = ChartBuilder.get_phasor_geometry(i_r_init, i_c_init)
        fig.add_trace(go.Scatter(x=xw_i, y=yw_i, mode='lines', fill='toself', fillcolor='lightgray', line=dict(width=0), hoverinfo='skip', visible=show_sweep, xaxis='x', yaxis='y'), row=1, col=1) # 4
        fig.add_trace(go.Scatter(x=xa_i, y=ya_i, mode='lines', line=dict(color='black', width=2), hoverinfo='skip', visible=show_sweep, xaxis='x', yaxis='y'), row=1, col=1) # 5

        # --- ONDAS SENOIDALES (Iniciales) ---
        t = np.linspace(0, 2 * np.pi, 200)
        u_wave = np.sin(t)
        angle_rad_init = np.arctan2(i_c_init, i_r_init if i_r_init > 0.0001 else 0.0001)
        i_wave_init = np.sin(t + angle_rad_init)
        
        fig.add_trace(go.Scatter(x=t, y=u_wave, mode='lines', line=dict(color='black', width=2), hoverinfo='skip', name='U'), row=2, col=1) # 6
        fig.add_trace(go.Scatter(x=t, y=i_wave_init, mode='lines', line=dict(color='#00B050', width=2, dash='dash'), hoverinfo='skip', name='I'), row=2, col=1) # 7

        # ==========================================
        # ANOTACIONES BASE ESTÁTICAS
        # ==========================================
        base_annotations = []
        
        base_annotations.append(dict(x=0.5, y=1.15, xref="x domain", yref="y domain", text="<b>Diagrama Fasorial</b>", showarrow=False, font=dict(size=14, color="black", family="Arial Black"), xanchor="center", yanchor="bottom"))
        base_annotations.append(dict(x=0.5, y=1.15, xref="x4 domain", yref="y4 domain", text="<b>Ondas Senoidales (U e I)</b>", showarrow=False, font=dict(size=14, color="black", family="Arial Black"), xanchor="center", yanchor="bottom"))
        base_annotations.append(dict(x=0.5, y=1.05, xref="x3 domain", yref="y3 domain", text="<b>Circuito Equivalente</b>", showarrow=False, font=dict(size=14, color="black", family="Arial Black"), xanchor="center", yanchor="bottom"))

        title_text = (
            f"<b>TAN DELTA ON {str(u_nominal).replace('.', ',')}KV</b><br>"
            f"<b>CLASS CABLE - NO ACTION GRAPHIC AREA FOR:</b><br>"
            f"<span style='font-size:12px'>U0 ≤ 4x10^-3 & [Tgδ@1.5U0 – Tgδ@0.5U0] ≤ {str(ref_delta_pivot).replace('.',',')}x10<sup>-3</sup></span>"
        )
        base_annotations.append(dict(
            x=0.5, y=1.05, xref="x2 domain", yref="y2 domain", 
            text=title_text, showarrow=False, font=dict(family="Arial", size=13, color="black"), 
            xanchor="center", yanchor="bottom", align="center"
        ))

        base_annotations.append(dict(x=0.15, y=0.85, xref="x2 domain", yref="y2 domain", text="<b>Zona No Action</b>", showarrow=False, font=dict(size=16, color="black", family="Arial Black"), bgcolor=Config.COLOR_AREA_FILL, borderpad=6, xanchor="center", yanchor="middle"))
        ...
        base_annotations.append(dict(x=c_x+0.05, y=0.815-offset_y, text="<b>I</b>", showarrow=False, font=dict(size=14, color="#00B050", family="Arial Black"), xref="x3", yref="y3", xanchor="left", yanchor="middle"))
        ...
        base_annotations.append(dict(x=c_x+0.05, y=0.61-offset_y, text="<b>Ic</b>", showarrow=False, font=dict(size=14, color="#0055D4", family="Arial Black"), xref="x3", yref="y3", xanchor="left", yanchor="middle"))
        ...
        base_annotations.append(dict(x=c_x+0.38, y=0.65-offset_y, text="<b>Ir</b>", showarrow=False, font=dict(size=14, color="#D50000", family="Arial Black"), xref="x3", yref="y3", xanchor="left", yanchor="middle"))
        ...
        # POSICIÓN DE LA FLECHA U (AJUSTADA A LA IZQUIERDA)
        base_annotations.append(dict(
            ax=c_x - 0.35, ay=0.7-offset_y,  
            x=c_x - 0.35, y=0.3-offset_y,    
            xref="x3", yref="y3", axref="x3", ayref="y3", 
            showarrow=True, arrowhead=2, arrowsize=1.5, arrowwidth=2, arrowcolor="black", text=""
        ))
        base_annotations.append(dict(
            x=c_x - 0.35, y=0.25-offset_y, 
            text="<b>U</b>", showarrow=False, 
            font=dict(size=18, color="black", family="Arial Black"), 
            xref="x3", yref="y3", xanchor="center", yanchor="top"
        ))
        ...
        base_annotations.append(dict(
            x=1.65, y=0.8, 
            text="Factor de Disipación<br>Tang δ = I<sub>R</sub> / I<sub>C</sub>", 
            showarrow=False, font=dict(size=11, color="black"), 
            xref="x", yref="y", 
            xanchor="right", yanchor="middle"
        ))

        # Ubicación estática de 'U' anclada a su curva de datos.
        base_annotations.append(dict(
            x=np.pi/2, y=1.1, text="<b>U</b>", 
            showarrow=False, font=dict(size=14, color="black", family="Arial Black"), 
            xref="x4", yref="y4", xanchor="center", yanchor="bottom"
        ))

        def get_dynamic_annots(i_r_val, i_c_val, curr_u0_disp, angle_rad=None):
            curr_u0_disp_str = str(curr_u0_disp).replace('.', ',')
            
            show_ir = i_r_val > 0.001
            safe_ir = i_r_val if i_r_val > 0.0001 else 0.0001
            
            annots = [
                dict(x=0.15, y=0.70, text=f"<b>Uo={curr_u0_disp_str}</b>", showarrow=False, font=dict(size=16, color="black", family="Arial Black"), bgcolor=Config.COLOR_BOX_U0, borderpad=6, xref="x2 domain", yref="y2 domain", xanchor="center", yanchor="middle"),
            ...
            # --- AGREGAR SÍMBOLO PHI Y FLECHA DOBLE EN LAS ONDAS ---
            if angle_rad is not None:
                # El cruce por cero hacia abajo de U es en PI.
                # El cruce por cero hacia abajo de I es en PI - angle_rad.
                desfase_x_end = np.pi 
                desfase_x_start = np.pi - angle_rad 
                
                # Flecha doble en el eje X
                if abs(desfase_x_end - desfase_x_start) > 0.05: # Mostrar solo si hay desfase notable
                    annots.append(dict(
                        x=desfase_x_end, y=0,             # Cabeza de la flecha en la curva U
                        ax=desfase_x_start, ay=0,         # Cola de la flecha en la curva I
                        xref="x4", yref="y4",
                        axref="x4", ayref="y4",
                        showarrow=True,
                        arrowhead=2,                      # Punta final
                        startarrowhead=2,                 # Punta inicial (para hacerla doble)
                        arrowsize=1.5,
                        arrowwidth=2,
                        arrowcolor="black",               
                        text=""
                ...
                # Símbolo Phi (φ) justo por encima de la flecha doble
                annots.append(dict(
                    x=(desfase_x_start + desfase_x_end)/2, y=0.1,
                    text="<b>φ</b>", 
                    showarrow=False, font=dict(size=20, color="black", family="Arial Black"), 
                    xref="x4", yref="y4", xanchor="center", yanchor="bottom"
                ))
                
                # Etiqueta 'I' dinámica siguiendo el pico máximo de su curva
                peak_x_i = np.pi/2 - angle_rad
                annots.append(dict(
                    x=peak_x_i, y=1.1, text="<b>I</b>", 
                    showarrow=False, font=dict(size=14, color="#00B050", family="Arial Black"),
                    xref="x4", yref="y4", xanchor="center", yanchor="bottom"
                ))
            ...
            if show_ir:
                ...
                annots.append(dict(x=0, y=0, text="", showarrow=False, xref="x", yref="y"))
                annots.append(dict(x=0, y=0, text="", showarrow=False, xref="x", yref="y"))
            
            return annots

        # ==========================================
        # GENERACIÓN DE FRAMES ANIMADOS Y BUCLE
        # ==========================================
        frames = []
        ...
        for i in range(num_frames):
            ...
            frame_data = [
                go.Scatter(x=v_points, y=curr_m_y),                                             # 0: Area Verde
                go.Scatter(x=v_points, y=curr_m_y, text=curr_m_labels, cliponaxis=False), # 1: Linea
                go.Scatter(x=[0, i_r], y=[i_c, i_c]),                           # 2: Linea horizontal superior
                go.Scatter(x=[i_r, i_r], y=[0, i_c]),                           # 3: Linea vertical 
                go.Scatter(x=xw, y=yw),                                         # 4: Cuña Delta
                go.Scatter(x=xa, y=ya),                                         # 5: Cuña Phi
                go.Scatter(x=t, y=u_wave),                                      # 6: Onda U
                go.Scatter(x=t, y=i_wave),                                      # 7: Onda I
            ]

            curr_u0_disp = round(curr_m_y[1], 1)
            curr_u0_disp = int(curr_u0_disp) if curr_u0_disp.is_integer() else curr_u0_disp
            
            f_annots = get_dynamic_annots(i_r, i_c, curr_u0_disp, angle_rad)
            frame_layout = go.Layout(annotations=base_annotations + f_annots)
        ...
        initial_u0_disp = int(u0_start) if u0_start.is_integer() else round(u0_start, 1)
        init_dyn_annots = get_dynamic_annots(i_r_init, i_c_init, initial_u0_disp, angle_rad_init)
        loop_sequence_area = [str(i) for i in range(num_frames)] * 100 
        
        ticks_v, ticks_stacked, _ = get_ticks_labels(v_points)

        # ==========================================
        # DISEÑO FINAL HORIZONTAL Y BLOQUEO DE EJES
        # ==========================================
        ...
        fig.update_layout(
            ...
            xaxis2=dict(
                domain=[0.35, 0.82], title="<b><br>NIVELES DE TENSION (kV)</b>", 
                tickmode='array', tickvals=v_points, ticktext=ticks_stacked, 
                range=[v_points[0], v_points[2]], 
                mirror=True, showline=True, linewidth=1, linecolor='black', showgrid=True, gridcolor='#E8E8E8', fixedrange=True,
                ticks="outside", ticklen=5, tickwidth=1, tickcolor="black"
            ),
            yaxis2=dict(
                domain=[0.0, 0.95], title="<b>TG (δ)<br>1·10⁻³</b>", range=[0, y_max], 
                showgrid=True, gridcolor='#E8E8E8', mirror=True, showline=True, linewidth=1, 
                linecolor='black', tickformat=",.1f", tickfont=dict(color="#666666"), fixedrange=True,
                ticks="outside", ticklen=5, tickwidth=1, tickcolor="black", side="left"
            ),
            ...
            yaxis4=dict(domain=[0.0, 0.35],
                range=[-1.5, 1.5], showgrid=True, gridcolor='#E8E8E8', zeroline=True, zerolinecolor='black', showticklabels=False, fixedrange=True),
            
            plot_bgcolor='white', paper_bgcolor='white', 
            
            height=750, margin=dict(l=40, r=60, t=140, b=100), showlegend=False,
...
def main():
    ...
    with st.sidebar.expander("Configuración Gráfico Tendencia", expanded=False):
        ...
        demo_u0_end_tendencia = st.number_input("Uo inicial", value=0.0, step=1.0, key="end_tendencia")
        m_10_val_tendencia = st.number_input("Uo final", min_value=0.0, max_value=20.0, value=4.0, key='m_10_tendencia', step=1.0, on_change=sync_mediciones_tendencia)
    ...
    with st.sidebar.expander("Configuración Zona No Action", expanded=False):
        ...
        p_delta_pivot = st.number_input("Delta Máximo Fijo (1.5Uo-0.5Uo)", value=1.5, step=0.1)
    ...
    if u_linea > 0:
        ...
        with tab_fan:
            ...
            with col2:
                ...
                st.plotly_chart(fig_fan, use_container_width=True, config={'displayModeBar': False, 'staticPlot': True}, key="tend_fan")
        ...
        with tab_area:
            ...
            st.plotly_chart(fig_area, use_container_width=True, config={'displayModeBar': False, 'staticPlot': True}, key="area_noaction")
            ...
            if st.button("Generar y Descargar GIF (Área No Action)"):
                with st.spinner("Procesando GIF..."):
                    try:
                        ...
                        st.download_button(label="Descargar GIF", data=gif_bytes, file_name="no_action.gif", mime="image/gif")
                    except Exception as e: st.error(f"Error: {e}")
