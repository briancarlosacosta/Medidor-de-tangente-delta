...
def main():
    ...
    with st.sidebar.expander("Ajuste eje X en kV", expanded=False):
        ...
        kv_start = st.number_input("Tensión Inicial (0,5x10⁻³Uo)", min_value=0.0,
                                   max_value=min(999.0, float(st.session_state.kv_mid_key)),
                                   format="%.2f", key='kv_start_key')

        kv_mid = st.number_input("Tensión Inicial (1,0x10⁻³Uo)", min_value=float(kv_start),

                                 max_value=min(999.0, float(st.session_state.kv_end_key)),
                                 format="%.2f", key='kv_mid_key')

        kv_end = st.number_input("Tensión Inicial (1,5x10⁻³Uo)", min_value=float(kv_mid),

                                 max_value=999.0, format="%.2f", key='kv_end_key')

        v_points = [kv_start, kv_mid, kv_end]
        x_dtick = st.number_input("Escala X (Saltos de)", min_value=0.1, max_value=999.0, value=1.0, step=0.5)
    ...
    if u_linea > 0:
        ...
        if modo_visualizacion == " Tendencia":
            ...
            with st.sidebar.expander("Configuración Gráfico Tendencia", expanded=True):
                u0_ref = st.number_input("Pivote del grafico de rectas de tendencia (1,0Uo x10⁻³)", min_value=0.0, max_value=4.0, value=2.0, step=0.1, key="u0_tend")
                mostrar_abanico = st.checkbox("Mostrar abanico histórico", value=True, key="chk_abanico")
        ...
        # ---------------------------------------------------------
            ...
            with st.sidebar.expander("Configuración limite máximo para no action", expanded=True):
                ...
                demo_u0_end_noaction = st.number_input("1,0Uo final (x10⁻³)", min_value=0.0, key="end_noaction", step=0.1, value=4.0, max_value=4.0)
                m_10_val_noaction = st.number_input("1,0Uo inicial (x10⁻³)", min_value=0.0, max_value=4.0, key='m_10_noaction_input', step=0.1)
