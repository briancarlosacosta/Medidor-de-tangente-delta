...
def get_ticks_labels(voltage_values: list):
    ...
    labels_stacked = [
        f"<b>{str(v05).replace('.', ',')}</b><br>0,5Uo",
        f"<b>{str(v10).replace('.', ',')}</b><br>1,0Uo",
        f"<b>{str(v15).replace('.', ',')}</b><br>1,5Uo"
    ]
...
class ChartBuilder:
    ...
    def _add_dynamic_titles_and_axis(fig: go.Figure, title_text: str, x_ticks: list, x_labels: list, num_steps: int = 50, y_max: float = 10.0, y_dtick: float = 1.0):
        ...
        fig.update_layout(
            autosize=True,
            font=dict(color="black"), # <--- 1. Fuerza texto negro global
            ...
            updatemenus=[{
                "buttons": [
                    # Bajamos la duración a 20ms (50 FPS) y APAGAMOS el redraw
                    {"args": [loop_sequence, {"frame": {"duration": 20, "redraw": False}, "fromcurrent": True, "transition": {"duration": 0}}], "label": "▶ PLAY", "method": "animate"},
                ...
                "font": {"color": "black"} # Texto de botones en negro
            }]
        )
    ...
    def draw_fan_animated(v_points, u0_measured, u_nominal, measured_y, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_fan=True, show_sweep=True):
        ...
        for i, v in enumerate(v_points):
            ...
            fig.add_annotation(
                ...
                text=f"{labels_u[i]}",showarrow=False, font=dict(size=14, color="black"),
        ...
        fig.add_annotation(
            x=v_points[1],    
            y=u0_measured,    
            text=f"ÁREA DE CUMPLIMIENTO 'NO ACTION' 1,0Uo= {u0_disp} x10⁻³",
            showarrow=True,
            arrowhead=0,
            arrowcolor="#A6A6A6",
            arrowwidth=1.5,
    ...
    def get_phasor_geometry(i_r, i_c):
        """siempre devuelve el mismo tamaño de array"""
    
        safe_ir = max(i_r, 0.0001) 
        angle = np.arctan2(i_c, safe_ir)
    ...
    def draw_area_shifting_demo(v_points, u_nominal, m_y_start, u0_end, ref_delta_pivot, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_limit_line=False, show_sweep=True):
        num_frames = 50 # <--- Volvemos a 50 para no ahogar al navegador
        ...
        y_exact_labels = [f"<b>{m_y_start[0]:.1f}</b>".replace('.', ',')]
        
        fig.add_trace(go.Scatter(x=[v_points[0]], y=[m_y_start[0]], mode='lines', line=dict(width=0), hoverinfo='skip', visible=show_sweep, xaxis='x2', yaxis='y2', cliponaxis=False), row=1, col=2)
        fig.add_trace(go.Scatter(x=[v_points[0]], y=[m_y_start[0]], mode='lines+markers+text', text=y_exact_labels, textposition="top center", marker=dict(size=12, symbol='diamond', color='white', line=dict(width=2, color='black')), line=dict(color=Config.COLOR_MEASURED, width=3), hoverinfo='skip', visible=show_sweep, xaxis='x2', yaxis='y2', cliponaxis=False), row=1, col=2)
        i_c_init = 1.2  
        # El valor inicial de Ir ya no es cero, es estrictamente proporcional al Tangente Delta inicial (u0_start)
        # Usamos un factor de escala (* 0.1) para que el vector sea visible frente al Ic de 1.2
        i_r_init = (u0_start * 0.1) if u0_start > 0.001 else 0.0001
        ...
        # --- NUEVO: Líneas verticales punteadas y etiquetas de Tensión ---
        base_annotations = []
        labels_u = ['0,5 Uo', '1,0 Uo', '1,5 Uo']
        for i, v in enumerate(v_points):
            # Dibujamos la línea punteada vertical de piso a techo
            static_shapes.append(
                dict(
                    type="line", 
                    x0=v, y0=0, x1=v, y1=y_max, 
                    xref="x2", yref="y2", 
                    line=dict(color="#B4C6A6", width=1.5, dash="dash"),
                    opacity=0.8
                )
            )
            # Agregamos el texto identificador arriba de cada línea
            base_annotations.append(
                dict(
                    x=v, y=y_max, 
                    xref="x2", yref="y2", 
                    text=f"<b>{labels_u[i]}</b>", 
                    showarrow=False, 
                    font=dict(size=13, color="#666666", family="Arial Black"),
                    xanchor="left" if i == 0 else ("center" if i == 1 else "right"),
                    yanchor="bottom",
                    yshift=8
                )
            )

        base_annotations.append(dict(x=0.5, y=1.08, xref="x domain", yref="paper", text="<b>Diagrama Fasorial</b>", showarrow=False, font=Config.TITLE_FONT, xanchor="center", yanchor="middle"))
        base_annotations.append(dict(x=0.5, y=1.15, xref="x4 domain", yref="y4 domain", text="<b>Desfase entre U e I </b>", showarrow=False, font=Config.TITLE_FONT, xanchor="center", yanchor="bottom"))
        base_annotations.append(dict(x=0.5, y=1.08, xref="x3 domain", yref="paper", text="<b>Circuito Equivalente</b>", showarrow=False, font=Config.TITLE_FONT, xanchor="center", yanchor="middle"))

        inequation_text = "<b>[(A + 5) ≥ C ≥ B]</b><br>" "Inecuación segun </b><br>" "limitaciones IEEE 400.2</B>"
        base_annotations.append(dict(

            x=0.5, y=0.15, xref="x3 domain", yref="y3 domain",
        ...
        title_text_main = ("<b>Analisis de Tangente Delta - 0,1Hz - IEE 400.2 </b></b><br>")

        base_annotations.append(dict(

            x=0.5, y=1.08, xref="x2 domain", yref="paper",
            text=title_text_main, showarrow=False, font=Config.TITLE_FONT,
            xanchor="center", yanchor="middle", align="center"

        ))

        base_annotations.append(dict(ax=c_x, ay=0.80-offset_y, x=c_x, y=0.70-offset_y, xref="x3", yref="y3", axref="x3", ayref="y3", showarrow=True, arrowhead=2, arrowsize=1.5, arrowwidth=2, arrowcolor="#00B050", text=""))
        base_annotations.append(dict(x=c_x+0.06, y=0.75-offset_y, text="<b>I</b>", showarrow=False, font=dict(size=14, color="#00B050", family="Arial Black"), xref="x3", yref="y3", xanchor="left", yanchor="middle"))
        ...
        base_annotations.append(dict(
            x=0.25, y=0.75,
            text="Factor de<br>Disipación<br> Tan (δ) = I<sub>R</sub> / I<sub>C</sub>",
            showarrow=False,
        ...
        def get_dynamic_annots(i_r_val, i_c_val, curr_u0_disp, angle_rad=None):
            show_ir = bool(i_r_val > 0.001)
            safe_ir = float(i_r_val) if i_r_val > 0.0001 else 0.0001
            
            # Inicializamos la lista de anotaciones SOLO con los datos del fasor
            annots = [
       ...
       # Inicializamos la traza de la "Curva Real Exponencial" en la gráfica base
        fig.add_trace(
            go.Scatter(
                x=[v_points[0]], y=[u0_start], 
                mode='lines', 
                # ACÁ ESTÁ EL CAMBIO: color negro y estilo punteado ('dot' o 'dash')
        ...
        for i in range(num_frames):
            ...
            current_x = v_points[0] + (v_points[2] - v_points[0]) * progress

            # 1. EL LÍMITE IDEAL LINEAL (Constante de 3 puntos)
            curr_x_arr = [v_points[0], v_points[0], v_points[0]]
            curr_y_arr = [u0_start, u0_start, u0_start]
            curr_m_labels = ["", "", ""]

            # 2. LA CURVA REAL EXPONENCIAL (Array de 50 puntos para fluidez WebGL)
            x_curve = np.linspace(v_points[1], v_points[2], 50)
            y_curve = np.full(50, u0_start)

            if current_x <= v_points[1]:
                # Fase 1: Ambas líneas viajan juntas y planas a u0_start
                curr_x_arr[1] = current_x; curr_x_arr[2] = current_x 
                current_y_val = u0_start

                x_curve = np.full(50, current_x)
                y_curve = np.full(50, u0_start)

                curr_m_labels[0] = f"<b>{u0_start:.1f}</b>".replace('.', ',')
                if current_x > v_points[0]:
                    curr_m_labels[1] = f"<b>{u0_start:.1f}</b>".replace('.', ',')
            else:
                # Fase 2: El límite sube recto, la curva sube exponencial
                curr_x_arr[1] = v_points[1]; curr_x_arr[2] = current_x
                
                # -- Límite Lineal (recta roja) --
                current_y_val = u0_start + 5.0 * ((current_x - v_points[1]) / (v_points[2] - v_points[1]))
                curr_y_arr[2] = current_y_val
                
                # -- Curva Exponencial Real (Naranja) --
                valid_idx = x_curve <= current_x
                x_valid = x_curve[valid_idx]
                
                # ========================================================
                # FUNCIÓN EXPONENCIAL REAL
                # Normalizamos x entre 0 y 1 para que represente tu (x - x0)
                norm_x = (x_valid - v_points[1]) / (v_points[2] - v_points[1])
                
                # A es el valor de Y en 1,0 Uo
                A = u0_start
                # Y_15 es el valor final al que debe llegar en 1,5 Uo
                Y_15 = u0_start + 5.0 
                
                # k = Ln(Y_15 / A)
                k = np.log(Y_15 / A)
                
                # Aplicamos tu ecuación: y = A * e^[k * (x - x0)]
                y_valid = A * np.exp(k * norm_x) 
                # ========================================================
                
                y_curve[valid_idx] = y_valid
                # Truco WebGL: el "futuro" de la curva copia el último valor válido para no saltar
                y_curve[~valid_idx] = y_valid[-1]
                x_curve[~valid_idx] = current_x

                curr_m_labels[0] = f"<b>{u0_start:.1f}</b>".replace('.', ',')
                curr_m_labels[1] = f"<b>{u0_start:.1f}</b>".replace('.', ',')
                curr_m_labels[2] = f"<b>{current_y_val:.1f}</b>".replace('.', ',')

            i_c = 1.2
            real_tip_up_current = y_curve[0] if current_x <= v_points[1] else y_valid[-1]
            i_r = real_tip_up_current * 0.1 if real_tip_up_current > 0.001 else 0.0001
            
            xw, yw, xa, ya, _, _, _, _ = ChartBuilder.get_phasor_geometry(i_r, i_c)
            angle_rad = np.arctan2(i_c, i_r)
            i_wave = np.sin(t + angle_rad)

            frame_data = [
                go.Scatter(x=curr_x_arr, y=curr_y_arr),
                go.Scatter(x=curr_x_arr, y=curr_y_arr, text=curr_m_labels, cliponaxis=False),
                ...
                # 3. AGREGAMOS LA NUEVA TRAZA AL FRAME (Traza N° 8)
                go.Scatter(x=x_curve, y=y_curve)
            ]
            
            # --- NUEVO: Etiquetas inteligentes para las curvas (TEXTOS VIAJEROS LARGOS) ---
           
            curve_annots = []
            
            # 1. Variables de texto (arrancan vacías por el efecto fantasma del STOP)
            txt_ideal = ""
            txt_real = ""
            txt_tip = ""
            
            # Solo llenamos los textos si la animación ya arrancó (Frame > 0)
            if current_x > v_points[0]:
                txt_ideal = "<b>IDEAL</b>"
                txt_real = "<b>REAL</b>"
                
            if current_x > v_points[1]:
                txt_tip = f"<b>{real_tip_up_current:.1f}</b>".replace('.', ',')

            # 2. Factor de viaje (85% detrás de la punta de dibujo) desde el INICIO
            travel_factor = 0.85
            dynamic_x = v_points[0] + (current_x - v_points[0]) * travel_factor
            
            ideal_x_val = dynamic_x
            real_x_val = dynamic_x
            
            # 3. Altura e inclinación dinámica según la fase del ensayo
            if dynamic_x <= v_points[1]:
                # FASE 1: Línea horizontal entre 0.5 y 1.0 Uo
                ideal_y_val = u0_start
                real_y_val = u0_start
                angulo_texto = 0  # Texto plano porque la línea es plana
            else:
                # FASE 2: Las curvas se separan y suben hacia 1.5 Uo
                # Altura para IDEAL
                ideal_y_val = u0_start + 5.0 * ((dynamic_x - v_points[1]) / (v_points[2] - v_points[1]))
                
                # Altura para REAL (Exponencial)
                norm_dynamic_x = (dynamic_x - v_points[1]) / (v_points[2] - v_points[1])
                A = u0_start
                Y_15 = u0_start + 5.0
                k = np.log(Y_15 / A)
                real_y_val = A * np.exp(k * norm_dynamic_x)
                
                angulo_texto = -40  # Inclinamos el texto para que siga la subida

            # 4. Inyectamos los diccionarios (IDEAL va por arriba, REAL por abajo)
            curve_annots.append(dict(
                x=ideal_x_val, y=ideal_y_val, xref="x2", yref="y2",
                text=txt_ideal, showarrow=False,
                font=dict(color="#D50000", size=11, family="Arial Black"),
                yanchor="bottom", yshift=8, xanchor="center", textangle=angulo_texto
            ))
                
            curve_annots.append(dict(
                x=real_x_val, y=real_y_val, xref="x2", yref="y2",
                text=txt_real, showarrow=False,
                font=dict(color="black", size=11, family="Arial Black"),
                yanchor="top", yshift=-5, xanchor="center", textangle=angulo_texto
            ))

            # El valor de la punta (diamante negro) sigue apareciendo en la Fase 2
            curve_annots.append(dict(
                x=current_x if current_x > v_points[1] else v_points[1], 
                y=real_tip_up_current, xref="x2", yref="y2",
                text=txt_tip, showarrow=False,
                font=dict(color="black", size=12, family="Arial Black"),
                yanchor="top", yshift=-10, xanchor="center"
            ))
            # -----------------------------------------------------
            f_annots = get_dynamic_annots(i_r, i_c, u0_start, angle_rad)
            frame_layout = go.Layout(annotations=base_annotations + f_annots + curve_annots, shapes=static_shapes)
            frames.append(go.Frame(data=frame_data, layout=frame_layout, name=str(i), traces=[0, 1, 2, 3, 4, 5, 6, 7, 8]))
        ...
        # Secuencia de 100 frames
        loop_sequence_area = [str(i) for i in range(num_frames)]*1
        ...
        x_margin = (v_points[2] - v_points[0]) * 0.05
        fig.update_layout(
            autosize=True,
            ...
            xaxis2=dict(domain=[0.35, 0.82],title="<b><br>Niveles de Tensión (kV)</b>",tickmode='array', tickvals=custom_ticks_area,ticktext=custom_labels_area,range=[v_points[0] - x_margin, v_points[2] + x_margin],mirror=True,showline=False,linewidth=1,linecolor='black',showgrid=True,gridcolor='#E8E8E8',fixedrange=True,ticks="outside",ticklen=5,tickwidth=1,tickcolor="black"),
            yaxis2=dict(domain=[0.0, 0.95],title="<b>TG (δ)<br>1·10⁻³</b>",range=[0, y_max],tickmode='linear',dtick=y_dtick, showgrid=True,gridcolor='#E8E8E8',mirror=True, showline=True,linewidth=1,linecolor='black',tickformat=",.1f",tickfont=dict(color="#666666"),fixedrange=True,ticks="outside",ticklen=5,tickwidth=1,tickcolor="black",side="left"),
            yaxis5=dict(domain=[0.0, 0.95],anchor='x2',overlaying='y2',side='right',range=[0, y_max],tickmode='linear',dtick=y_dtick,showgrid=False,showline=False,tickformat=",.1f",tickfont=dict(color="#666666"),fixedrange=True,ticks="outside",ticklen=5,tickwidth=1,tickcolor="black"),
            xaxis3=dict(domain=[0.85, 1.0], range=[0.0, 1.2], showgrid=False, zeroline=False, showticklabels=False, fixedrange=True),
            yaxis3=dict(domain=[0.0, 0.95], range=[0.0, 1.25], showgrid=False, zeroline=False, showticklabels=False, fixedrange=True),
            xaxis4=dict(domain=[0.0, 0.28], range=[0, 2*np.pi], showgrid=False, gridcolor='#E8E8E8', zeroline=True, zerolinecolor='black', showticklabels=False, fixedrange=True),
            yaxis4=dict(domain=[0.0, 0.35], range=[-1.5, 1.5], showgrid=False, gridcolor='#E8E8E8', zeroline=True, zerolinecolor='black', showticklabels=False, fixedrange=True),
            plot_bgcolor='white', paper_bgcolor='white', height=750, margin=dict(l=40, r=60, t=140, b=100), showlegend=False,

          updatemenus=[{
                "buttons": [
                    # Volvemos a tu lógica original impecable: redraw en True y transition activada
                    ...
                    {"args": [[None], {"frame": {"duration": 0, "redraw": False}, "mode": "immediate", "transition": {"duration": 0}}], "label": "⏸ PAUSA", "method": "animate"},
                    {"args": [["0"], {"frame": {"duration": 0, "redraw": True}, "mode": "immediate", "transition": {"duration": 0}}], "label": "⏹ STOP", "method": "animate"}
                ],
                "direction": "left", "pad": {"r": 10, "t": 10}, "showactive": False, "type": "buttons", "x": 0.12, "xanchor": "center", "y": -0.08, "yanchor": "top"
            }]
        )
        fig.add_trace(go.Scatter(x=[v_points[0]], y=[0], mode='none', xaxis='x2', yaxis='y5', showlegend=False, hoverinfo='skip'))
...
def main():
    st.markdown(
        ...
        """
        <style>
        [data-testid="stSidebarContent"] { padding-right: 1rem; padding-left: 1rem; }
        .main-header { font-size: 1.2rem; font-weight: bold; margin-bottom: 0.5rem; }
        /* Pestañas adaptativas al modo oscuro/claro */
        div[role="radiogroup"] { flex-direction: row; gap: 4px; }
        ...
        div[role="radiogroup"] label {
            ...
            border-bottom: none;
            color: inherit !important; /* Hereda blanco en dark mode, negro en light */
        ...
        div[role="radiogroup"] label[aria-checked="true"] {
            ...
            color: black !important; /* Texto oscuro siempre en la pestaña activa para contraste */
            font-weight: bold;
            border-top: 3px solid #D50000 !important;
        }
    ...
    st.markdown('<p class="main-header">Modo de Visualización</p>', unsafe_allow_html=True)
    ...
    st.markdown("<hr style='margin-top: 0; border-color: rgba(128, 128, 128, 0.3);'>", unsafe_allow_html=True)

    # --- SIDEBAR: PARÁMETROS TÉCNICOS ---
    st.sidebar.header("Parámetros de Ensayo")

    def recalcular_ejes_x():

        v05, u0, v15 = calculate_voltages(st.session_state.u_linea_input)
        st.session_state.kv_start_key = v05
        st.session_state.kv_mid_key = u0
        st.session_state.kv_end_key = v15

    u_linea = st.sidebar.number_input(
        "Tensión Nominal (U) [kV]",
        min_value=0.0, max_value=999.0, value=33.0, step=0.1,
        key='u_linea_input', on_change=recalcular_ejes_x
    )

    v05_def, u0_def, v15_def = calculate_voltages(u_linea)
    if 'kv_start_key' not in st.session_state: st.session_state.kv_start_key = v05_def
    if 'kv_mid_key' not in st.session_state: st.session_state.kv_mid_key = u0_def
    if 'kv_end_key' not in st.session_state: st.session_state.kv_end_key = v15_def

    with st.sidebar.expander("Ajuste eje X en kV", expanded=False):

        kv_start = st.number_input("Tensión (0,5Uo)", min_value=0.0,
                                   max_value=min(999.0, float(st.session_state.kv_mid_key)),
                                   format="%.2f", key='kv_start_key')

        kv_mid = st.number_input("Tensión (1,0Uo)", min_value=float(kv_start),

                                 max_value=min(999.0, float(st.session_state.kv_end_key)),
                                 format="%.2f", key='kv_mid_key')

        kv_end = st.number_input("Tensión  (1,5Uo)", min_value=float(kv_mid),
    ...
    with st.sidebar.expander("Ajuste eje Y (Tag δ)", expanded=False):
        y_max = st.number_input(
            ...
            value=10.0, 
            step=1.0,
            key="y_max_fija" # Key única para evitar conflictos
        )
        y_dtick = st.number_input(
            "Escala eje Y (Saltos de)", 
            min_value=0.1, 
            max_value=15.0, 
            value=1.0, 
            step=0.5,
    ...
    if u_linea > 0:
        ...
        if modo_visualizacion == " Tendencia":

            with st.sidebar.expander("Configuración Gráfico Tendencia", expanded=True):
                u0_ref = st.number_input("Pivote del grafico de rectas de tendencia (Tag (δ)x10⁻³x1,0Uo)", min_value=0.0, max_value=4.0, value=2.0, step=0.1, key="u0_tend")
                mostrar_abanico = st.checkbox("Mostrar abanico", value=True, key="chk_abanico")
                mostrar_barrido_tend = st.checkbox("Mostrar barrido", value=True, key="chk_barrido_tend")
                m_10_val_tendencia = float(u0_ref)
                m_y_tendencia = [m_10_val_tendencia, m_10_val_tendencia, m_10_val_tendencia + 5.0]
            ...
            with col2:
                ...
                if st.button("Generar y Descargar GIF (Tendencia)", key="btn_gif_tend"):
        ...
        # ---------------------------------------------------------
        ...
        elif modo_visualizacion == " Límite máximo para no action":

            mostrar_barrido_noact = True

            with st.sidebar.expander("Configuración limite máximo para no action", expanded=True):
                p_delta_pivot = 1.5
                
                
                # Forzamos el mínimo y el valor por defecto a 0.1 
                m_10_val_noaction = st.number_input(
                    "Tag (δ)x10⁻³x1,0Uo (inicial)", 
                    min_value=0.1, 
                    max_value=4.0, 
                    value=0.1, 
                    key='m_10_noaction_input', 
                    step=0.1
                )

                # El valor final lo dejamos fijo internamente en 4.0 
                u0_end_anim = 4.0
            ...
            if st.button("Generar y Descargar GIF (Limite máximo para no action)", key="btn_gif_noact"):
