...
class ChartBuilder:
    ...
    def draw_area_shifting_demo(v_points, u_nominal, m_y_start, u0_end, ref_delta_pivot, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_limit_line=False, show_sweep=True):
        ...
        fig.update_layout(
            ...
            xaxis2=dict(domain=[0.35, 0.82], title="<b><br>Niveles de Tensión (kV)</b>",tickmode='array', tickvals=custom_ticks_area, ticktext=custom_labels_area,range=[v_points[0], v_points[2]], mirror=True, showline=False, linewidth=1, linecolor='black', showgrid=True, gridcolor='#E8E8E8', fixedrange=True, ticks="outside", ticklen=5, tickwidth=1, tickcolor="black"),
            yaxis2=dict(domain=[0.0, 0.95], title="<b>TG (δ)<br>1·10⁻³</b>", range=[0, y_max],tickmode='linear',dtick=y_dtick, showgrid=False,mirror=True, showline=True,linewidth=1,linecolor='black',tickformat=",.1f",tickfont=dict(color="#666666"),fixedrange=True,ticks="outside",ticklen=5,tickwidth=1,tickcolor="black",side="left"),
    ...
    if u_linea > 0:
        ...
        # ---------------------------------------------------------
            ...
            with st.sidebar.expander("Configuración limite máximo para no action", expanded=True):
                ...
                demo_u0_end_noaction = st.number_input("Tag (δ)x10⁻³x1,0Uo (final)", min_value=0.0, key="end_noaction", step=0.1, value=4.0, max_value=4.0)
                m_10_val_noaction = st.number_input("Tag (δ)x10⁻³x1,0Uo (inicial)", min_value=0.0, max_value=4.0, key='m_10_noaction_input', step=0.1)
