...
class ChartBuilder:
    ...
    def _add_dynamic_titles_and_axis(fig: go.Figure, title_text: str, x_ticks: list, x_labels: list, num_steps: int = 50, y_max: float = 10.0, y_dtick: float = 1.0):
        ...
        fig.update_layout(
            ...
            yaxis=dict(title="<b>Tag(δ) 1·10⁻³</b>", range=[0, y_max], dtick=y_dtick, gridcolor='#E0E0E0', mirror=True, linecolor='black'),
    ...
    def draw_area_shifting_demo(v_points, u_nominal, m_y_start, u0_end, ref_delta_pivot, y_max=10.0, y_dtick=1.0, x_dtick=1.0, show_limit_line=False, show_sweep=True):
        ...
        fig.add_trace(go.Scatter(x=v_points, y=m_y_start, mode='lines', fill='tozeroy', line=dict(width=0), fillcolor='rgba(0,0,0,0)', hoverinfo='skip', visible=show_sweep, xaxis='x2', yaxis='y2', cliponaxis=False), row=1, col=2) # 0
        ...
        # ==========================================
        # ANOTACIONES BASE ESTÁTICAS (TITULOS UNIFICADOS)
        # ==========================================
        base_annotations = []
        ...
        # MODIFICACIÓN DE TÍTULO PRINCIPAL (UNIFICADO)
        title_text_main = (
        ...
        base_annotations.append(dict(
            x=1.65, y=0.8, 
            text="<br> <br> Factor <br> de <br>   Disipación Tan (δ) = I<sub>R</sub> / I<sub>C</sub>", 
        ...
        # MODIFICACIÓN SOLICITADA: UBICACIÓN DE CUADROS AZUL/ROJO/GRIS
        def get_dynamic_annots(i_r_val, i_c_val, curr_u0_disp, angle_rad=None):
            curr_u0_disp_str = str(curr_u0_disp).replace('.', ',')
            curr_u0_plus_5 = str(round(curr_u0_disp + 5.0, 1)).replace('.', ',')
            
            show_ir = i_r_val > 0.001
            safe_ir = i_r_val if i_r_val > 0.0001 else 0.0001
            
            # Posición ajustada para alinear con la parte superior del gráfico
            pos_x_lateral = 0.31 

            annots = [
                # Cuadro 0,5Uo (Gris Azulado) - Arriba
                dict(x=pos_x_lateral, y=0.93, text=f"<b>0,5Uo={curr_u0_disp_str} x10⁻³</b>", 
                     showarrow=False, font=dict(size=12, color="black", family="Arial Black"), 
                     bgcolor=Config.COLOR_BOX_05, borderpad=6, xref="paper", yref="paper", 
                     xanchor="right", yanchor="middle"),
                
                # Cuadro 1,0Uo (Azul Inducor) - Centro
                dict(x=pos_x_lateral, y=0.85, text=f"<b>1,0Uo={curr_u0_disp_str} x10⁻³</b>", 
                     showarrow=False, font=dict(size=12, color="white", family="Arial Black"), 
                     bgcolor=Config.COLOR_BOX_10, borderpad=6, xref="paper", yref="paper", 
                     xanchor="right", yanchor="middle"),
                
                # Cuadro 1,5Uo (Rojo Inducor) - Abajo
                dict(x=pos_x_lateral, y=0.77, text=f"<b>1,5Uo={curr_u0_plus_5} x10⁻³</b>", 
                     showarrow=False, font=dict(size=12, color="white", family="Arial Black"), 
                     bgcolor=Config.COLOR_BOX_15, borderpad=6, xref="paper", yref="paper", 
                     xanchor="right", yanchor="middle"),
        ...
        for i in range(num_frames):
            ...
            frame_data = [
                go.Scatter(x=v_points, y=curr_m_y, fill='tozeroy', fillcolor='rgba(0,0,0,0)'), 
        ...
        fig.update_layout(
            ...
            xaxis2=dict(domain=[0.35, 0.82], title="<b><br>Niveles de Tensión (kV)</b>", tickmode='array', tickvals=v_points, ticktext=ticks_stacked, range=[v_points[0], v_points[2]], mirror=True, showline=True, linewidth=1, linecolor='black', showgrid=True, gridcolor='#E8E8E8', fixedrange=True, ticks="outside", ticklen=5, tickwidth=1, tickcolor="black"),
...
def get_excel(voltages, measured, u_nom):
    output = BytesIO()
    df = pd.DataFrame({"Nivel": ["0.5 Uo", "1.0 Uo", "1.5 Uo"], "kV": voltages, "Tg Delta": measured})
...
def main():
    ...
    with st.sidebar.expander("Configuración Gráfico Tendencia", expanded=False):
        u0_ref = st.number_input("Ajuste el pivote del grafico de tendencia (1,0Uo x10⁻³)", min_value=0.0, max_value=10.0, step=0.1)
        ...
        demo_u0_end_tendencia = st.number_input("1,0Uo inicial (x10⁻³)", min_value=0.0, step=0.1, key="end_tendencia")
        m_10_val_tendencia = st.number_input("1,0Uo final (x10⁻³)", min_value=0.0, max_value=20.0, key='m_10_tendencia', step=0.1, on_change=sync_mediciones_tendencia)

        m_05_calc_tendencia = float(m_10_val_tendencia)
        m_15_calc_tendencia = float(m_10_val_tendencia + 5.0)

        st.number_input("Medido @ 0,5x10⁻³ Uo (Automático)", value=m_05_calc_tendencia, disabled=True, key="05_tendencia")
        st.number_input("Medido @ 1,5x10⁻³ Uo (Automático)", value=m_15_calc_tendencia, disabled=True, key="15_tendencia")
        m_y_tendencia = [m_05_calc_tendencia, m_10_val_tendencia, m_15_calc_tendencia]

    with st.sidebar.expander("Configuración limite máximo para no action", expanded=False):
        p_delta_pivot = st.number_input("(1,5x10⁻³Uo - 0,5x10⁻³Uo)", value=1.5, step=0.1)
        mostrar_barrido_noact = st.checkbox("Mostrar barrido (Recta roja)", value=True, key="chk_barrido_noact")
        
        st.subheader("Simulación")
        demo_u0_end_noaction = st.number_input("1,0Uo final (x10⁻³)", key="end_noaction", step=0.1)
        ...
        st.number_input("Medido @ 0,5x10⁻³ Uo (Automático)", value=m_05_calc_noaction, disabled=True, key="05_noaction")
        st.number_input("Medido @ 1,5x10⁻³ Uo (Automático)", value=m_15_calc_noaction, disabled=True, key="15_noaction")
    ...
    if u_linea > 0:
        tab_fan, tab_area = st.tabs(["Tendencia ", "Limite máximo para no action"])
        ...
        with tab_area:
            ...
            fig_area = ChartBuilder.draw_area_shifting_demo(v_points, u_linea, m_y_start_anim, u0_end_anim, p_delta_pivot, y_max=y_max, y_dtick=y_dtick, x_dtick=x_dtick, show_limit_line=False, show_sweep=mostrar_barrido_noact)
